...
- **Scroll Handling**: We ensure that the chat scrolls to the latest message using `useRef` and `useEffect`. This provides a smooth user experience by automatically scrolling to the newest message.
...
- **Styling**: We use Mantine's `Card` component for consistent styling, ensuring a clean and uniform look for the chat messages. This makes our text stream visually appealing!
